import os
from segmentation_adjusted.dataset import DiffusionDataset
from segmentation_adjusted.augmenter import Augmenter
from segmentation_adjusted.discriminative import DiscriminativeSubNetwork
...
class SegLit(pl.LightningModule):
    def __init__(
        ...
        self.train_ds = DiffusionDataset(conf_data["train_folder_images"], self.image_size, self.channels)
        self.val_ds = DiffusionDataset(conf_data["val_folder_images"], self.image_size, self.channels)
        self.test_ds = DiffusionDataset(conf_data["test_folder_images"], self.image_size, self.channels)
    ...
    def get_reconstruction(self, batch):
        ...
        rec = self.reconstruction_model.reconstruct(batch, self.noise_amount, 3)
        return rec

    def forward(self, batch):
        reconstruction = self.get_reconstruction(batch["image"])
        concatenated_inputs = torch.cat([batch["image"], reconstruction], axis=1)
    ...
    def shared_step(self, batch, mode):
        ...
        mask = mask.long()
        # labels = (mask.sum(dim=0) != 0)
        concatenated_inputs = torch.cat([batch["image"], reconstruction], axis=1)
        prediction = self.segmentation_network(concatenated_inputs)

        loss = self.loss(prediction, mask.squeeze(1))
        prediction = torch.nn.functional.softmax(prediction, dim=1)

        anomaly_map = prediction[:, 1, ...].unsqueeze(1)

        self.log(f"{mode}_loss", loss, on_step=False, on_epoch=True)
        return loss, augmented, reconstruction, anomaly_map, mask.float()
    ...
    def validation_step(self, batch, batch_idx):
        loss, augmented, reconstruction, anomaly_map, mask = self.shared_step(batch, "val")

        if self.current_epoch % self.log_every_n_epoch == 0:
            for i in range(1):
                imgs = map(TF.to_pil_image, [augmented[i], reconstruction[i], mask[i], anomaly_map[i]])
                self.logger.log_image(key=f"train-{self.current_epoch}", images=list(imgs), caption=["image", "reconstruction", "mask", "anomaly_map"])
